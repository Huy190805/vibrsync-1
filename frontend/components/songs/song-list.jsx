...
export default function SongList({ songs: propSongs }) {
  const { playSong, isPlaying, currentSong, togglePlayPause, nextSong, setSongs, setContext, setContextId } = useMusic();
  ...
  const handleBlock = () => setOptionsOpenId(null);
  ...
  return (
    <div className="bg-[#2f1047] shadow-md rounded-xl overflow-hidden relative border border-[#250f35]">
      <div
        ref={scrollRef}
        className="max-h-[360px] overflow-y-auto transition-all duration-300 scrollbar-thin scrollbar-thumb-[#250f35] scrollbar-track-[#2f1047]"
        onScroll={() => {
            ...
            {propSongs.map((song, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-3 text-neutral-400 hidden md:table-cell">{song.album || "N/A"}</td>
                  <td className="p-3 text-neutral-400 hidden md:table-cell">{formatDuration(song.duration || 0)}</td>
      ...
      {optionsOpenId && (
        ...
        >
          ...
          <div className="mt-4 border-t border-[#30114a] pt-3">
            <SongActionsMenu song={propSongs.find((s) => s.id === optionsOpenId)} onClose={() => setOptionsOpenId(null)} />
            <ul className="text-sm mt-2 space-y-2">
              <li onClick={handleLyrics} className="hover:bg-[#30114a] rounded p-2 cursor-pointer">Lyrics</li>
              <li onClick={handlePlayNext} className="hover:bg-[#30114a] rounded p-2 cursor-pointer">Play Next</li>
              <li onClick={handleBlock} className="hover:bg-[#30114a] rounded p-2 cursor-pointer">Block</li>
              <li onClick={handleCopyLink} className="hover:bg-[#30114a] rounded p-2 cursor-pointer">Copy Link</li>
            </ul>
          </div>
        </div>
      )}
