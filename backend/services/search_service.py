...
class SearchService:
    ...
    @staticmethod
    def lyrics_lrc_to_text(lyrics_lrc: str) -> str:
        if not lyrics_lrc:
            return ""
        # Bỏ timestamp [mm:ss.xx]
        clean = re.sub(r"\[\d{2}:\d{2}(?:\.\d{2})?\]", "", lyrics_lrc)
            ...
            try:
                songs = songs_collection.find({
                    "$or": [
                        ...
                        {"artist": regex},
                        {"lyrics": regex},
                        {"normalizedLyrics": normalized_regex},
                        # fallback: search trực tiếp lyrics_lrc
                        {"lyrics_lrc": regex},
                        {"lyrics_lrc": normalized_regex},
                        # ✅ search theo thể loại
                        {"genre": regex},
                        {"normalizedGenre": normalized_regex},
                    ]
                }, {
